...
baseName = '../Abaqus_Output/LIModulus_FxHFB_eta_zero_Model_'; % FE output file

% Initalize the output
output = zeros(size(pData, 1), 7);
...
for j = 1:size(pData, 1)
    ...
    solBVP = bvp5c(@(z, u)Lin_govDiffEq(z, u, p), @(u0, uL) Lin_FxHFB_BC(u0, uL, p), init_guess, opts);
...
writematrix(output, 'X_LIMod_FxHFB.xlsx');
