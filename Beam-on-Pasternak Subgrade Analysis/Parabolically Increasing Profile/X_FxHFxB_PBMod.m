...
% pile embeded in soil with parabolically increasing modulus and eta = 0. 
%
...
baseName = '../Abaqus_Output/PBModulus_FxHFxB_eta_zero_Model_'; % FE output file
...
for j = 1:size(pData, 1)
    ...
    solFEMfileName = baseName + string(j-1) + '.csv'; % the index is j-1, the abaqus automation script uses python and python uses 0 as the first index
    ...
    solBVP = bvp5c(@(z, u) PB_govDiffEq(z, u, p), @(u0, uL) PB_FxHFxB_BC(u0, uL, p), init_guess, opts);
...
writematrix(output, 'X_PBMod_FxHFxB.xlsx');
